...
/**
 * This class extends the finch.Robot class to incorporate functions to control the inputs and
 * outputs of the finch.Finch. It includes methods to set the values of motors and LEDs, as well as
 * methods to read the values of the sensors.
 *
 * <p>Krissie Lauwers, BirdBrain Technologies LLC October 2019
 */
public class Finch extends Robot {
    ...
    /**
     * This function sends a request to BlueBird Connector to determine whether or not the device is
     * a finch.Finch.
     */
    private boolean isFinch() {
        StringBuilder newURL = new StringBuilder(baseUrl);
        String testURL = (newURL.append("in/isFinch/static/").append(deviceInstance)).toString();

        String stringResponse = sendHttpRequest(testURL);
        if (stringResponse.equals("false")) {
            System.out.println("Error: Device " + deviceInstance + " is not a finch.Finch");
            return false;
    ...
    /**
     * Formats the direction string for sending to the bluebird connector. If the selection made is
     * not acceptable, returns 'Neither'.
     *
     * @param direction
     * @return
     */
    ...
    /**
     * Sends a request for the finch to move forward or backward a given distance at a given speed.
    ...
    public void setMotors(double leftSpeed, double rightSpeed) {
        ...
        String[] urlArgs = {
            "out", "wheels", deviceInstance, Double.toString(leftSpeed), Double.toString(rightSpeed)
        };
        String url = getUrl(urlArgs);
        httpRequestOut(url);
    }
    ...
    /**
     * Get the current value of the specified finch line sensor. Return value is inverted (100 -
     * value) so that more reflected light = bigger number
     *
     * @param direction - R or L to specify right or left
     * @return - brightness as a value 0-100
     */
    ...
    /**
     * getOrientation() provides information about the finch's current orientation. This function
     * overrides the function in the finch.Robot class so that results are in the finch reference
     * frame.
     *
