...
/**
 * This is an abstract class that is inherited by finch.Microbit.java, finch.Hummingbird.java and
 * finch.Finch.java. It includes methods to print on the micro:bit LED array or set those LEDs
 * individually. It also contains methods to read the values of the micro:bit accelerometer and
 * magnetometer.
 *
...
abstract class Robot {
     ...
     * etc.) on the micro:bit and finch.Hummingbird. */
    ...
    protected boolean httpRequestInBoolean(String URLRequest) {
        String stringResponse = sendHttpRequest(URLRequest);
        if (!stringResponse.equalsIgnoreCase("true") && !stringResponse.equalsIgnoreCase("false")) {
            System.out.println("Error: " + stringResponse);
            return false;
        } else {
    ...
    public void print(String message) {
        ...
        // Warn the user if there are any special characters. Note that we don't use
        // isCharacterOrDigit() because we can only display English characters
        char letter;
    ...
    public void setDisplay(int[] ledValues) {
         ...
         */
        for (int i = 0; i < ledLen; i++) {
            if (ledValues[i] == 1) displayStatus[i] = true;
            else displayStatus[i] = false;
        }
    ...
    public void setPoint(int row, int column, int value) {
         ...
         */
        if (value == 1) displayStatus[position] = true;
    ...
    public void playNote(int note, double beats) {
        ...
        String[] urlArgs = {
            "out", "playnote", Integer.toString(note), Integer.toString((int) beats), deviceInstance
        };
    ...
    /**
     * getButton() takes in a button and checks whether it is pressed. The function shows a warning
    ...
    /**
     * getTemperature() returns the current temperature in degrees Celcius from the micro:bit
     * temperature sensor
     *
    ...
    /**
     * getOrientationBoolean checks whether the device currently being held to a specific
     * orientation or shaken.
     *
